...
function setupDefaultIniSettings()
{
	...
	if ( fs.existsSync( SETTINGS_INI ) )
	{
		...
		if ( !defaultSettings.Server_Settings.LOCAL_API_END_POINT )
		{
			...
			settings['LOCAL_API_END_POINT'] = 'http://localhost/docteur-aoun.com/api/';
			settings['LOCAL_PROJECT_URL'] = 'http://localhost/docteur-aoun.com/';
	...
	// Server_Settings
	settings = {
		API_END_POINT: 'https://docteur-aoun.com/api/',
		PROJECT_URL: 'https://docteur-aoun.com/'
	};
	fini.writeSync('settings', settings, 'Server_Settings');
}
// Read ini file
...
ipcMain.on('create-window', (e, arg) =>
{
	...
	e.sender.send('window-created', JSON.stringify(win));
});
// Close window
